...
<details>
<summary><b>🔧 一次性配置：创建 Chrome 调试快捷方式（点此展开）</b></summary>

1. **完全关闭 Chrome**（任务管理器确认无残留进程）

2. **PowerShell 启动调试模式**——用独立配置目录，不影响你日常使用的 Chrome：

```powershell
Start-Process "C:\Program Files\Google\Chrome\Application\chrome.exe" -ArgumentList @("--remote-debugging-port=9222", "--remote-allow-origins=*", "--user-data-dir=$env:LOCALAPPDATA\ChromeDebug", "https://www.douyin.com")
```

3. 弹出的新 Chrome 窗口中**登录抖音**

> 💡 桌面已放 `start_chrome_debug.bat` 双击即可，以后每次先启动这个 Chrome 再运行爬虫。
...
1. 确认 Chrome 调试模式已启动——PowerShell 运行 `netstat -an | findstr 9222` 应有输出
2. 确认调试 Chrome 中已登录抖音（`not_exist_login_cookie: false`）
3. 如果 WebSocket 连接报 403，检查启动命令是否包含 `--remote-allow-origins=*`
