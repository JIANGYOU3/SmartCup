...
### 2026-07-14 — 采集链路修复与 CDP 工作流完善

- 修复知乎、抖音搜索结果 CSV 不能正确交给详情爬虫的问题。
- 让知乎 AI 标签器兼容当前爬虫输出字段；修复专栏标题、失败重试和空 CSV 处理。
- 优化抖音多批次合并：优先保留含评论的完整记录，避免中间文件重复参与合并。
- 增加 `websocket-client` 依赖，并完善 Chrome CDP 连接失败时的安全处理。
- 在 README、完整教程和 AI 项目说明中加入 Windows Chrome 调试模式启动、验证和排错流程。
- 完成真实冒烟测试：DeepSeek、知乎搜索、抖音 CDP 搜索均已通过。

完整记录见 [2026-07-14 工作日志](docs/工作日志_20260714.md)。
...
> ⚠️ **重要**：抖音搜索 API 有严格风控，推荐使用 **CDP 浏览器搜索**（方案一）。直接 API 搜索大概率被拦截。

---

#### 方案一：CDP 浏览器搜索（⭐ 推荐）

**原理**：在已登录的 Chrome 浏览器中执行搜索，利用浏览器内置环境自动处理 Cookie、签名和反爬。需要 Chrome 以调试模式运行。

<details>
<summary><b>🔧 每次抖音关键词搜索前：启动 Chrome 调试模式（点此展开）</b></summary>

1. **完全关闭此前用于调试的 Chrome 窗口**。保留日常 Chrome 没问题，但必须用下面命令启动一个独立的调试浏览器。

2. **Windows PowerShell 启动调试模式**——`google-chrome` 是 Linux 命令，Windows 请使用 `chrome.exe`。以下命令使用独立配置目录，不影响日常浏览器：
...
```
   ...
   若 Chrome 安装在 32 位目录，将 `$env:ProgramFiles` 改为 `${env:ProgramFiles(x86)}`。没有 Chrome 时也可把路径改为 Edge 的 `msedge.exe`；两者都支持 CDP。

3. 在弹出的新窗口中**登录抖音并保持 `www.douyin.com` 页面打开**。不要关闭这个窗口，随后再在项目终端运行抖音命令。

4. **可选验证**（在项目的 Linux/WSL 终端运行）。输出 `READY` 才表示爬虫可连接浏览器：

```bash
conda run -n SmartCup python -c "from source.douyin_crawler.lib.cdp2 import get_ws; print('READY' if get_ws() else 'NOT_RUNNING')"
```

> ⚠️ `--remote-allow-origins=*` 只用于这个临时调试浏览器。不要用它启动存有日常账号和敏感信息的 Chrome 配置。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--keywords` / `-k` | （必需） | 搜索关键词，逗号分隔 |
| `--pages` / `-p` | `5` | 每个关键词翻页数（每页 20 条） |
| `--with-comments` | `False` | 同时抓取评论（点赞数/回复数/子回复） |
| `--comment-pages` | `5` | 评论翻页数（每页 20 条） |
| `--search-only` | `False` | 仅搜索不爬取 |
...
| 策略 | 方式 | 说明 |
|------|------|------|
| **A（优先）** | CDP 浏览器 `fetch()` 调搜索 API | 利用 Chrome 内置环境，无需签名，**推荐** |
| **B（补充）** | API `/aweme/v1/web/aweme/detail/` | 需要 a_bogus + websign 签名（搜索仍可能被风控） |
| **C（回退）** | 解析 HTML SSR 数据 | 抖音已改为客户端渲染，此策略已基本失效 |

- a_bogus 签名：纯 Python 查表法实现（`sign/abogus_pure.py`），无需 Node.js
- websign 签名：Node.js 补环境（`lib/runtime/`），用于特定 API 端点
...
| `评论N时间` | 评论时间 |
| `评论N属地` | IP 属地 |
| `评论N子回复M` | 子回复内容（M=1,2,3） |
| `评论JSON` | 完整评论数据的 JSON（备用） |
...
- **CDP 浏览器搜索**：利用 Chrome DevTools Protocol 在浏览器中执行 fetch，绕过反爬签名
- **三策略解析**：CDP 搜索 + API 补充 + HTML SSR 回退，保证数据完整性
...
1. 确认使用的是**独立调试 Chrome**，且命令包含 `--remote-debugging-port=9222` 和 `--remote-allow-origins=*`
2. 确认调试 Chrome 中已登录抖音，并保持 `www.douyin.com` 页面打开
3. 在项目终端运行上方的 `get_ws()` 验证命令，必须输出 `READY`
4. 若 `NOT_RUNNING`，检查 PowerShell 命令中的 Chrome 路径；Windows 不能使用 `google-chrome`
5. 若 WebSocket 连接报 403，重启调试 Chrome，并确认包含 `--remote-allow-origins=*`

**如果使用 API 搜索**：

1. Cookie 过期（抖音 Cookie 有效期很短，需频繁更新）
2. a_bogus 签名映射表过期（更新 `sign/data/time_mapping.json`）
3. 搜索接口被风控（`verify_check`）——改用 CDP 浏览器搜索
