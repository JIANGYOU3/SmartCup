...
# execjs 为可选依赖，仅在使用 JS 签名回退时才需要
try:
    from utils.execjs_fix import execjs  # type: ignore
    _HAS_EXECJS = True
except ImportError:
...
class Request(object):
    ...
    def getJSON(self, uri: str, params: dict, data: dict = None, live=None, web2=None):
        ...
        referer_map = {
            ...
            '/aweme/v1/web/general/search/single/': f"https://www.douyin.com/search/{quote(str(params.get('keyword', '')))}?type=general",
            '/aweme/v1/web/search/item/': f"https://www.douyin.com/search/{quote(str(params.get('keyword', '')))}?type=general",
